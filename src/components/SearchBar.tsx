    ...
    <div className="relative">
      <div className="relative">
        {isCurrentCategorySearch ? (
          <Tag className={`absolute left-3 ${largerMobile ? 'top-3.5' : 'top-2.5'} ${largerMobile ? 'h-6 w-6' : 'h-5 w-5'} text-blue-500`} />
        ) : (
        ...
        <input
          ...
          className={`pl-10 w-full ${largerMobile ? 'p-3 text-lg' : 'p-2'} border-2 ${isCurrentCategorySearch ? 'border-blue-400' : 'border-gray-400'} rounded-full search-modal-input ${modalSearch ? 'modal-search-input' : ''}`}
          value={searchTerm}
          onChange={(e) => {
