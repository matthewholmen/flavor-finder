...
Version 2.1 - Parses "esp." qualifiers and cocktail ingredients for better beer/spirit data.
"""
...
# Cocktail category mappings (for generic cocktail descriptions)
COCKTAIL_SPIRIT_MAP = {
    'brown spirit': ['Bourbon', 'Whiskey', 'Brandy', 'Dark Rum'],
    'white spirit': ['Vodka', 'Gin', 'White Rum', 'Tequila'],
    'dark spirit': ['Bourbon', 'Whiskey', 'Brandy', 'Dark Rum'],
    'light spirit': ['Vodka', 'Gin', 'White Rum'],
    'citrus': ['Vodka', 'Gin'],
    'sweet': ['Rum', 'Brandy'],
}
...
def normalize_drink(name):
    ...
    # IMPORTANT: Don't remove commas if this is a "cocktails made with X, Y, Z" line
    if 'cocktail' not in name or 'made with' not in name:
        name = re.sub(r',.*$', '', name)  # Remove everything after comma (for single drinks)
    ...
    # Don't remove "or X" if it's a cocktail ingredient list
    if 'cocktail' not in name or 'made with' not in name:
    ...
    while i < len(lines):
        ...
        if is_potential_header:
            ...
            if matched:
                ...
                elif matched == "chicken":  # DEBUG
                    print(f"DEBUG: Re-encountered chicken <- {ingredient_raw}")
            else:
                in_pairing_section = False
                current_ingredient = None

        # Extract drink pairings (indented lines - space or form feed)
        elif in_pairing_section and current_ingredient and line and (raw_line.startswith(' ') or raw_line.startswith('\f')):
            drink = line.strip()

            # Check if this line is a sub-section header vs emphasized drink name
            text_only = re.sub(r'[^A-Za-z]', '', drink)
            if text_only.isupper() and len(text_only) > 2:
                # Check if it contains drink keywords (wine, beer, cocktail, etc.) or known varietals
                drink_lower = drink.lower()
                has_drink_keywords = any(keyword in drink_lower for keyword in [
                    'wine', 'beer', 'cocktail', 'champagne', 'whiskey', 'vodka',
                    'gin', 'rum', 'tequila', 'sake', 'cognac', 'brandy', 'port',
                    'sherry', 'vermouth', 'liqueur', 'tea', 'coffee', 'juice',
                    # Common wine varietals/types
                    'chardonnay', 'cabernet', 'merlot', 'pinot', 'sauvignon',
                    'riesling', 'syrah', 'shiraz', 'zinfandel', 'malbec',
                    'rosé', 'burgundy', 'bordeaux', 'chianti', 'rioja',
                    'prosecco', 'cava', 'moscato', 'gewürztraminer'
                ])

                # If no drink keywords, likely a subsection header - skip it
                if not has_drink_keywords:
                    i += 1
                    continue
            ...
            if i + 1 < len(lines):
                ...
                # Merge if:
                # 1. Current line ends with comma or "or" (incomplete)
                # 2. Next line is indented (starts with space in raw form)
                # 3. Next line is not all uppercase (not a new section header)
                # 4. Next line doesn't look like a complete drink entry (no "wine", "beer", etc.)
                next_raw = lines[i + 1]
                is_continuation = (
                    (drink.endswith(',') or drink.endswith('or')) and
                    next_raw.startswith(' ') and
                    next_line and
                    not (len(re.sub(r'[^A-Za-z]', '', next_line)) > 2 and re.sub(r'[^A-Za-z]', '', next_line).isupper()) and
                    not any(x in next_line.lower() for x in ['wine', 'beer', 'ale', 'cocktail', 'juice'])
                )
                if is_continuation:
                    drink = drink + ' ' + next_line
            ...
            if is_non_alcoholic:
                ...
                # Beer - Enhanced with "esp." parsing
                added = False

                # Check for "esp." qualifiers (e.g., "beer, esp. lager or pale ale")
                if 'esp' in drink:
                    # Extract the "esp." part
                    esp_match = re.search(r'esp\.?\s+(.+)$', drink, re.IGNORECASE)
                    if esp_match:
                        esp_text = esp_match.group(1).lower()
                        # Split by "or" or commas
                        esp_styles = re.split(r'\s+or\s+|,\s*', esp_text)

                        for style_text in esp_styles:
                            style_text = style_text.strip()
                            # Match against BEER_STYLES
                            for beer_type, beer_style in BEER_STYLES.items():
                                if beer_type in style_text:
                ...
                # Also check main text for beer types
                if not added:
                ...
                # NEW: Handle "beer, STYLE" comma pattern (e.g., "beer, wheat" → Wheat Beer)
                if not added and 'beer,' in drink_normalized:
                    # Extract what comes after "beer,"
                    beer_comma_match = re.search(r'beer,\s+(\w+)', drink_normalized)
                    if beer_comma_match:
                        modifier = beer_comma_match.group(1).strip()
                        # Match modifier against beer style keywords
                        for beer_type, style in BEER_STYLES.items():
                            # Check if modifier is a key word in beer_type (e.g., "wheat" in "wheat beer")
                            if modifier in beer_type.split():
                                if not any(b['name'].lower() == beer_type for b in pairings[current_ingredient]['beers']):
                                    pairings[current_ingredient]['beers'].append({
                                        'name': beer_type.title(),
                                        'style': style
                ...
                # Generic beer fallback (only if no specific styles found)
                if not added and 'beer' in drink_normalized:
                ...
                # Spirit - Enhanced with cocktail ingredient parsing
                added = False

                # Check for "cocktails made with X, Y, Z"
                if 'cocktail' in drink_normalized:
                    made_with_match = re.search(r'made with (.+)$', drink, re.IGNORECASE)
                    if made_with_match:
                        # Extract list of spirits
                        spirit_text = made_with_match.group(1).lower()
                        # Split by commas, "or", "and" (handle ", or" patterns)
                        spirit_names = re.split(r',\s*(?:or\s+)?|,?\s+(?:or|and)\s+', spirit_text)

                        for spirit_name in spirit_names:
                            spirit_name = spirit_name.strip()
                            # Match against SPIRIT_STYLES
                            # Use exact match to avoid "cognac" matching "armagnac"
                            if spirit_name in SPIRIT_STYLES:
                                spirit_style = SPIRIT_STYLES[spirit_name]
                                # Skip liqueurs in favor of base spirits (unless it's the only match)
                                if spirit_style != 'liqueur' or True:  # Keep all for now
                                    if not any(s['name'].lower() == spirit_name for s in pairings[current_ingredient]['spirits']):
                                        pairings[current_ingredient]['spirits'].append({
                                            'name': spirit_name.title(),
                                            'style': spirit_style
                                        })
                                        if current_ingredient == "chicken":  # DEBUG
                                            print(f"  DEBUG: Added spirit '{spirit_name.title()}' to chicken")
                                        added = True

                    # Check for "cocktail with brown/white spirit"
                    elif 'with' in drink_normalized:
                        for category, spirit_list in COCKTAIL_SPIRIT_MAP.items():
                            if category in drink_normalized:
                                for spirit_name in spirit_list:
                                    spirit_type = spirit_name.lower()
                                    if spirit_type in SPIRIT_STYLES:
                                        if not any(s['name'].lower() == spirit_type for s in pairings[current_ingredient]['spirits']):
                                            pairings[current_ingredient]['spirits'].append({
                                                'name': spirit_name,
                                                'style': SPIRIT_STYLES[spirit_type]
                                            })
                                            added = True

                # Also check for direct spirit mentions
                if not added:
                ...
                # Generic cocktail fallback (only if no specific spirits found)
...
def main():
    print("=" * 60)
    print("DRINK PAIRING EXTRACTION - Version 2.1")
    print("=" * 60)
    print()
