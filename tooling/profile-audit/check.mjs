...
import { TEXTURES, FUNCTIONS, TASTE_DIMS, MAX_TEXTURES, MAX_FUNCTIONS } from './vocab.mjs';
...
for (const term of [...TEXTURES, ...FUNCTIONS]) {
...
for (const file of batchFiles) {
  ...
  for (const e of entries) {
    ...
    if (!Array.isArray(e.textures)) errors.push(`${where}: textures must be an array`);
      ...
      if (e.textures.length === 0) noTexture++;
    }
    ...
    if (!Array.isArray(e.functions)) errors.push(`${where}: functions must be an array`);
      ...
      if (e.functions.length === 0) noFunction++;
    }
...
const report = [
  ...
  `- No texture tags (texture-neutral): ${noTexture}`,
  `- No function tags: ${noFunction}`,
